...
def display_professional_results_table(
    results: list, panel_metrics: Optional[dict] = None
) -> None:
    ...
    print(f"{Colors.BOLD}{Colors.BRIGHT_CYAN}{title:^{table_width}}" f"{Colors.RESET}")
    print(f"{Colors.BOLD}{'=' * table_width}{Colors.RESET}")
    print(
        f"{Colors.BOLD}┌{'─' * 6}┬{'─' * 25}┬{'─' * 16}┬{'─' * 16}"
        f"┬{'─' * 16}┐{Colors.RESET}"
    )
    print(
        f"{Colors.BOLD}│ {'Rank':^4} │ {'Model':^23} │ {'R² Score':^14} │"
        f" {'MAE':^14} │ {'RMSE':^14} │{Colors.RESET}"
    )
    print(
        f"{Colors.BOLD}├{'─' * 6}┼{'─' * 25}┼{'─' * 16}┼{'─' * 16}┼{'─' * 16}┤{Colors.RESET}"
    )
    ...
    for rank, result in enumerate(all_results_sorted, 1):
        ...
        print(
            f"│ {rank:^4} │ {model_name:<23} │ {r2_str:^14} │ {mae_str:^14} │ {rmse_str:^14} │"
        )
    print(
        f"{Colors.BOLD}└{'─' * 6}┴{'─' * 25}┴{'─' * 16}┴{'─' * 16}┴{'─' * 16}┘{Colors.RESET}"
    )
    ...
    if all_results_sorted:
        ...
        print(
            f"{Colors.BOLD}{Colors.BRIGHT_CYAN}Best Model: {best_model}{Colors.RESET}"
        )
        print(f"{Colors.BOLD}R² = {best_r2:.4f}{Colors.RESET}")
        if not np.isnan(best_mae):
            print(
                f"{Colors.BOLD}MAE = {best_mae:.4f}, RMSE = {best_rmse:.4f}{Colors.RESET}\n"
            )
        else:
...
def run_data_preparation() -> None:
    """Load, clean, merge indicators, save train/test/full datasets.

    Splits: 1996-2017 train, 2018-2023 test.
    """
    ...
    try:
        ...
        print(
            f"\n{Colors.CYAN}Datasets:{Colors.RESET} "
        ...
        if isinstance(e, FileNotFoundError):
            print(
                f"{Colors.CYAN}Run [0] Check Environment to verify data files{Colors.RESET}"
            )
        pause()
        return
...
def train_all_ml_models():
    ...
    print_info(
        "Training Dynamic Panel model (econometric model with fixed effects)...\n"
    )
    ...
    global TRAINING_COMPLETED_THIS_SESSION, WORKFLOW_COMPLETION
    global SESSION_MODELS_DATA, SESSION_TRAIN_DATA
    global SESSION_TEST_DATA, SESSION_FULL_DATA
...
def display_nickell_bias(bias_metrics: Dict[str, float]) -> None:
    """Display Nickell (1981) bias analysis for dynamic panel models.

    Shows bias analysis for fixed effects models.
    """
    print(f"\n{Colors.BOLD}{Colors.BRIGHT_WHITE}{'='*80}{Colors.RESET}")
    print(
        f"{Colors.BOLD}{Colors.BRIGHT_YELLOW}"
        f"NICKELL BIAS ANALYSIS (Econometric Diagnostic){Colors.RESET}"
    )
    ...
    print(
        f"\n{Colors.BRIGHT_CYAN}Bias Severity:{Colors.RESET} "
        f"{severity_color}{severity_icon} {interpretation}{Colors.RESET}"
    )

    if not np.isnan(bias_metrics["half_life_ols"]) and not np.isnan(
        bias_metrics["half_life_corrected"]
    ):
        ...
        print(
            f"  {Colors.DIM}> Shocks to political stability decay by 50% "
            f"in ~{hl_corr:.1f} years{Colors.RESET}"
        )
...
def display_diagnostic_tests(diagnostics: Dict[str, Dict]) -> None:
    ...
    print(f"\n{Colors.BOLD}{Colors.BRIGHT_WHITE}{'='*80}{Colors.RESET}")
    print(
        f"{Colors.BOLD}{Colors.BRIGHT_YELLOW}ECONOMETRIC DIAGNOSTIC TESTS{Colors.RESET}"
    )
    print(f"{Colors.BOLD}{Colors.BRIGHT_WHITE}{'='*80}{Colors.RESET}")
    ...
    if "hausman" in diagnostics:
        ...
        if "error" in hausman:
            ...
            print(
                f"  {Colors.DIM}Using Fixed Effects (standard for panel data){Colors.RESET}"
            )
        else:
    ...
    if "autocorrelation" in diagnostics:
        ...
        if "error" in autocorr:
            ...
            print(f"  Conclusion: {color}{icon} {conclusion}{Colors.RESET}")
            print(
                f"  {Colors.DIM}Note: Model uses clustered SE to handle "
                f"potential autocorrelation{Colors.RESET}"
            )
    ...
    if "heteroscedasticity" in diagnostics:
        ...
        if "error" in hetero:
            ...
            print(
                f"  {Colors.DIM}Note: Model uses clustered SE to handle "
                f"potential heteroscedasticity{Colors.RESET}"
            )
...
def train_dynamic_panel(skip_input: bool = False) -> Optional[Dict[str, float]]:
    ...
    try:
        predictors = [
            ...
            if col
            not in ["political_stability", "Country Name", "Year", "Country Code"]
        ]
...
def display_overfitting_analysis(models_data: list) -> None:
    ...
    print(
        f"{Colors.BOLD}{Colors.BRIGHT_YELLOW}"
        f"OVERFITTING ANALYSIS (ML Models Diagnostic){Colors.RESET}"
    )
    print(f"{Colors.BOLD}{Colors.BRIGHT_WHITE}{'='*80}{Colors.RESET}")

    print(
        f"\n{Colors.BOLD}{'Model':<25} {'Train R²':>10} {'Test R²':>10} "
        f"{'Gap':>10} {'Status':>15}{Colors.RESET}"
    )
    print(f"{Colors.DIM}{'-'*80}{Colors.RESET}")
    ...
    for model in models_data:
        ...
        print(
            f"{model_name:<25} {train_r2:>10.4f} {test_r2:>10.4f} "
            f"{gap:>9.4f}  {status_color}{status_icon} "
            f"{status_text}{Colors.RESET}"
        )

    print(f"\n{Colors.DIM}Legend:{Colors.RESET}")
    print(
        f"  {Colors.BRIGHT_GREEN}[OK] EXCELLENT{Colors.RESET}  : "
        f"Gap < 0.05  (Model generalizes very well)"
    )
    print(
        f"  {Colors.YELLOW}[!] ACCEPTABLE{Colors.RESET} : "
        f"Gap < 0.10  (Slight overfitting, still usable)"
    )
    print(
        f"  {Colors.RED}[X] OVERFITTING{Colors.RESET}: "
        f"Gap >= 0.10 (Model memorizes training data)"
    )
    print(f"{Colors.BOLD}{Colors.BRIGHT_WHITE}{'='*80}{Colors.RESET}\n")
...
def test_all_models():
    ...
    successful_models = [
        m for m in SESSION_MODELS_DATA if "error" not in m and "r2" in m
    ]
    ...
    if failed_models:
        ...
        for m in failed_models:
            print(
                f"{Colors.RED}  {m['model']}: {m.get('error', 'Unknown error')}{Colors.RESET}"
            )
...
def run_visualization():
    ...
    print(
        f"  {Colors.BOLD}{Colors.BRIGHT_GREEN}[1]{Colors.RESET}  Generate All Visualizations"
    )
    print(
        f"  {Colors.BOLD}{Colors.BRIGHT_MAGENTA}[2]{Colors.RESET}  Copy Figures to Desktop"
    )
    print(
        f"  {Colors.BOLD}{Colors.BRIGHT_MAGENTA}[3]{Colors.RESET}  Show Hyperparameters"
    )
...
def call_viz_function(func: Any) -> None:
    ...
    global SESSION_MODELS_DATA, SESSION_TRAIN_DATA, SESSION_TEST_DATA
    global SESSION_FULL_DATA, SESSION_PANEL_METRICS
    ...
    try:
        func_display = func.__name__.replace("generate_", "").replace("_", " ")
        print(
            f"{Colors.DIM}  Generating {func_display}...{Colors.RESET}",
            end=" ",
        )
        if func.__name__ in [
            "generate_distribution_plot",
            ...
            func(
                ...
                SESSION_TEST_DATA,
                figures_dir,
                panel_metrics=SESSION_PANEL_METRICS,
            )
...
def test_coverage() -> None:
    ...
    try:
        ...
        print_error("Test execution timed out (>15 minutes)")
    except FileNotFoundError:
        print_error(
            "pytest or coverage not found! Install: pip install pytest pytest-cov"
        )
    except Exception as e:
...
def show_main_menu() -> None:
    ...
    def menu_item(number, key, title, description):
        """Format menu item with completion status.

        Green if completed, cyan if pending.
        """
        completed = WORKFLOW_COMPLETION.get(key, False)
        color = Colors.BRIGHT_GREEN if completed else Colors.BRIGHT_CYAN
        return (
            f"  {Colors.BOLD}{color}[{number}]{Colors.RESET}  "
            f"{title:24s}{Colors.DIM}->{Colors.RESET}  {description}"
        )
